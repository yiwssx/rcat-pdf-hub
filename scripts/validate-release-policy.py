...
assert (ROOT / "apps/web/playwright.config.ts").exists(), "Missing Playwright configuration"
assert (ROOT / "apps/web/tests/e2e/pdf-hub.smoke.spec.ts").exists(), "Missing PDF Hub browser smoke tests"
...
# The current UI must not regress to the superseded console/auth-state behavior.
web_app = read("apps/web/app/components/pdf-hub-app.tsx")
assert "const me = await getMe(value);\n      setIdentity(me);\n      setAuth(value);" in web_app
