...
def main() -> int:
    ...
    for record in records:
        try:
            ...
            actual_size, actual_hash = digest(path)
    ...
    if settings.storage_backend == "local":
        ...
        for orphan in sorted(actual - expected_local):
            failures.append(f"orphan storage file not referenced by DB: {orphan}")
    else:
        ...
        for orphan in sorted(actual_keys - expected_keys):
            failures.append(f"orphan S3 object not referenced by DB: {orphan}")

    print(f"restored-data records={len(records)} verified={verified} failures={len(failures)}")
