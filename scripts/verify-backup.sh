...
# Read only the fixed keys written by backup.sh; do not source arbitrary shell content.
...
[ "${backup_format}" = "1" ] || { echo "Unsupported backup format: ${backup_format}" >&2; exit 1; }
...
case "${storage_backend}" in
  local)
    archive="${BACKUP_DIR}/pdf-data.tar.gz"
    ;;
  s3)
    archive="${BACKUP_DIR}/s3-objects.tar.gz"
    ;;
  *)
    echo "Unsupported storage backend in manifest: ${storage_backend}" >&2
    exit 1
    ;;
esac
