...
export function PdfHubConsole() {
  ...
  async function run(operation: string) {
    if (!targetId && operation !== "merge") return setMessage("เลือกไฟล์เป้าหมายก่อน");
    switch (operation) {
      case "merge":
        if (files.length < 2) return setMessage("Merge ต้องมีอย่างน้อย 2 ไฟล์");
    ...
    <main className="shell">
      <header className="hero">
        <div>
          ...
          <h1>PDF Hub</h1>
          <p>PDF processing, OCR ไทย, SSO/LDAP, malware scanning, S3/NAS, observability และ document archive จากศูนย์กลางเดียว</p>
        </div>
      ...
      {authenticated && integrations && (
        <section className="panel">
          <div className="panelTitle"><h2>Platform</h2><span>Phase 3</span></div>
          <div className="tools">
            ...
            <div><strong>Metrics</strong><small>{integrations.prometheus_enabled ? "Prometheus enabled" : "disabled"}</small></div>
        ...
        <>
          <section className="workspaceGrid">
            <div className="panel drop">
              ...
              <label htmlFor="files"><span className="dropIcon">＋</span><strong>เพิ่มไฟล์เข้า Workspace</strong><span>PDF และเอกสาร Office/LibreOffice</span></label>
            </div>
          ...
          <section className="panel">
            ...
            <div className="tools">
              <button disabled={busy || files.length < 2} onClick={() => void run("merge")}><strong>รวม PDF</strong><small>รวมทุกไฟล์ใน workspace</small></button>
              <button disabled={busy || !targetId} onClick={() => void run("ocr")}><strong>OCR ไทย + อังกฤษ</strong><small>Tesseract tha+eng</small></button>
              <button disabled={busy || !targetId} onClick={() => void run("compress")}><strong>บีบอัด PDF</strong><small>Ghostscript</small></button>
              <button disabled={busy || !targetId} onClick={() => void run("pdfa")}><strong>PDF/A-2</strong><small>เอกสารเก็บถาวร</small></button>
              ...
              {integrations?.paperless_enabled && <button disabled={busy || !targetId} onClick={archive}><strong>Archive</strong><small>ส่งเข้า Paperless-ngx</small></button>}
            </div>
          </section>
          ...
          <section className="grid two advancedGrid">
            <div className="panel toolConfig">
              ...
              <button className="primary" onClick={() => void run("split")} disabled={busy || !targetId}>แยก/เลือกหน้า</button>
              ...
              <button className="secondary" onClick={() => void run("rotate")} disabled={busy || !targetId}>หมุนหน้า</button>
            </div>
            ...
            <div className="panel toolConfig">
              ...
              <button className="primary" onClick={() => void run("watermark")} disabled={busy || !targetId}>ใส่ Watermark</button>
            </div>
            ...
            <div className="panel toolConfig">
              ...
              <button className="primary" onClick={() => void run("page-numbers")} disabled={busy || !targetId}>ใส่เลขหน้า</button>
            </div>

            <div className="panel toolConfig">
              <div className="panelTitle"><h2>PDF Stamp</h2><span>overlay</span></div>
              <label>ไฟล์ Stamp<select value={stampId} onChange={(e) => setStampId(e.target.value)}><option value="">— เลือก PDF —</option>{files.filter((file) => file.id !== targetId).map((file) => <option value={file.id} key={file.id}>{file.original_name}</option>)}</select></label>
              ...
              <button className="primary" onClick={() => void run("stamp")} disabled={busy || !targetId || !stampId}>ประทับ PDF</button>
            </div>
          </section>
